...
import { RubroArticulo, RubroArticuloDocument } from './schemas/rubro-articulo.schema';
...
export class RubroArticuloService {
    ...
    async create( createRubroArticuloDto: CreateRubroArticuloDto ) {
        ...
        try {
            const rubroArticulo = await this.rubroArticuloModel.create(createRubroArticuloDto);
    ...
    private handleExceptions( error: any ) {
        ...
        console.log(error);
        throw new InternalServerErrorException(`Can't create rubroArticulo - Check server logs`);
      }
