...
import {
  ...
  RubroProductoManufacturadoDocument,
} from './schemas/rubro-producto-manufacturado.schema';
...
export class RubroProductoManufacturadoService {
  ...
  async create(
    ...
    createRubroProductoManufacturadoDto.nombre =
      createRubroProductoManufacturadoDto.nombre.toUpperCase();

    try {
      const rubroProductoManuacturado =
        await this.rubroProductoManufacturadoModel.create(
  ...
  private handleExceptions(error: any) {
    if (error.code === 11000) {
      throw new BadRequestException(
        `rubroProductoManufacturado exists in db ${JSON.stringify(
          error.keyValue,
        )}`,
      );
    }
    ...
    throw new InternalServerErrorException(
      `Can't create rubroProductoManufacturado - Check server logs`,
    );
  }
}
